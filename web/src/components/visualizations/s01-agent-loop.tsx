...
const STEP_INFO = [
  { title: "While 循环", desc: "每个 Agent 都是一个 while 循环，持续调用模型直到它说 'stop'" },
  { title: "用户输入", desc: "用户发送消息时，循环开始" },
  { title: "调用模型", desc: "将所有消息发送给 LLM，它看到一切并决定做什么" },
  { title: "stop_reason: tool_use", desc: "模型想要使用工具，循环继续" },
  { title: "执行并追加", desc: "运行工具，将结果追加到 messages[]，再喂回去" },
  { title: "再次循环", desc: "相同代码路径，第二次迭代。模型决定编辑文件" },
